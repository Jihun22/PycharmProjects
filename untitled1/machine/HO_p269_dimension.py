...
X_centered = X - X.mean(axis=0)  # PCA는 데이터의 평균이 0이라고 가정하고 있음
U, s, V = np.linalg.svd(X_centered)  # covariance matrix를 Decomposition하여 USVt 3개 amtrix로  구분한 결과

c1 = V.T[:, 0]   # V amtrix는 eigenvextor를 포함하고 있는 matrix. V를 Transpose하여 첫번째 vector를 출력
c2 = V.T[:, 1]   # V amtrix는 eigenvextor를 포함하고 있는 matrix. V를 Transpose하여 두번째 vector를 출력

S = np.zeros(X.shape)
S[:n, :n] = np.diag(s)

np.allclose(X, U.dot(S).dot(V))  # numpy는 np.allclose(a, b)를 사용하여 a와 b가 같은 객체를 가릐고 있는지 확인
T = X.dot(V.T[:, :2])

#-----------------------------------------------------------------
# PCA using scikit-learn사용하기
# page 280
#-----------------------------------------------------------------
from sklearn.decomposition import PCA
...
pca = PCA()
pca.fit(X)
d = np.argmax(np.cumsum(pca.explained_variance_ratio_) >= 0.95) + 1  # variance_ratio_) >= 0.95가되는 PC개를 구함
